...
dff <- df %>%
  ...
  mutate(problem = case_when(
    max_temp >= 36 & fvfm >= 0.3 & f <= 200 ~ "abnormally high w/ low Ft",
...
vals <- initmods %>%
  ...
  mutate(problem = factor(problem, levels = c("none", "abnormally high", "abnormally high w/ low Ft", 
                                              "high residual from fitted", "high cook's distance")))
...
plotfits <- function(data) {
  ggplot(data = data, aes(x = max_temp)) +
    ...
    facet_wrap(~ geno) +
    scale_color_manual(values = c("black", "red", "orange", "blue", "turquoise"), drop = FALSE)
}
...
dfff <- vals %>%
  mutate(problem = case_when(.cooksd > 0.25 & .resid > 0 ~ "high cook's distance", 
                             .resid > 0.15 ~ "high residual"
                             TRUE ~ as.character(problem))) %>%
  mutate(problem = factor(problem, levels = c("none", "abnormally high", 
                                              "abnormally high w/ low Ft", 
                                              "high residual", "high cook's distance"))) %>%
  mutate(fvfm = case_when(problem == "high residual from fitted" ~ NA_real_, 
                          problem == "high cook's distance" ~ NA_real_, TRUE ~ fvfm)) %>%
...
fpars <- fmods %>% 
  ...
  unnest(pars) %>%
  filter(term == "ED50")
#write_csv(fpars, path = "indfits_lessconstr.csv")
...
m3f <- drm(fvfm ~ max_temp, genoN, data = dfff,
          fct = LL.3(names = c('hill', 'max', 'ED50')),
          lowerl = c(20, NA, 30),
          upperl = c(100, NA, 40),    # upper limit of 100 on hill parameter
          pmodels = data.frame(genoN, genoN, genoN))  # all genos have indep. maximum
