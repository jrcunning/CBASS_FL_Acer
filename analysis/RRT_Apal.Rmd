---
title: "Reef Renewal Tavernier *A. palmata* CBASS"
...
b1log <- readlog("data/raw/20201006_RRT_Apal/temperature/sdlog/20201007_BOX1_LOG.TXT", prefix = "B1")
b2log <- readlog("data/raw/20201006_RRT_Apal/temperature/sdlog/20201006_BOX2_LOG.TXT", prefix = "B2")
b3log <- readlog("data/raw/20201006_RRT_Apal/temperature/sdlog/20201007_BOX3_LOG.TXT", prefix = "B3")

# Tidy arduino log files for this run
arduinolog <- bind_rows(b1log, b2log, b3log) %>%
  # Filter to just the date of this run
  filter(date(dttm) == "2020-10-06") %>%
  # Specify which arduino boxes controlled port vs. starboard tank sets
  mutate(tank_set = if_else(str_detect(tank, "B3"), "port", "starboard")) %>%
...
hobologfiles <- list.files("data/raw/20201006_RRT_Apal/temperature/hobo", pattern = ".xlsx", full.names = TRUE)
...
inkbird_sp <- read_csv("data/program_files/inkbird_settings.csv") %>%
  ...
  mutate(max_temp = as.numeric(str_extract(max_temp, "[0-9]+")),
         dttm = ymd_hms(paste("2020-10-06", time)), hm = format(dttm, "%H:%M"),
         temp = NA) %>%
  left_join(read_csv("data/tank_setup.csv")) %>%
  select(dttm, hm, sp, temp, max_temp, tank_set, tank_id)
...
ggplot(templog, mapping = aes(x = dttm, y = temp, group = tank_id)) +
  ...
  scale_x_datetime(breaks = "hours", labels = label_date("%H:%M"),
                   limits = as.POSIXct(c("2020-10-06 12:00:00", "2020-10-06 21:00:00"), tz = "UTC")) +
  theme_hc() +
...
path <- "data/raw/20201006_RRT_Apal/PAM/dpii.xlsx"
dpii <- path %>%
  excel_sheets() %>%
  set_names() %>%
  map_df(~ read_excel(path = path, sheet = .x, range = "A1:F43", col_types = "text"), .id = "tank_id") %>%
  mutate(f = as.numeric(f), fm = as.numeric(fm), fvfm = as.numeric(yii)) %>%
  clean_names()

# Combine with tank_id metadata
tanks <- read_csv("data/tank_setup.csv")
dpii <- full_join(dpii, tanks) %>%
  select(geno, rec, f, fm, fvfm, tank_id, tank_set, max_temp)
```
...
out2 <- full_join(out, out1, 
                  by = c("nursery", "geno", "f", "fm", "fvfm", "max_temp")) %>%
  mutate(is_out = is_out.x | is_out.y) %>%
  select(nursery, geno, f, fm, fvfm, max_temp, is_out)
...
df <- df %>%
  ...
  mutate(problem = case_when(
    ...
    #max_temp >= 36 & fvfm >= 0.3 & f <= 250 ~ "abnormally high w/ low Ft",
  ...
  mutate(fvfm = case_when(
    ...
    problem == "none" ~ fvfm)) %>%
  select(nursery, geno, f, fm, fvfm, max_temp, fvfmraw, problem)
...
```{r}
fvals %>%
  distinct(geno, ed50, std.error) %>%
  drop_na() %>%
  mutate(geno = fct_reorder(geno, ed50)) %>%
  ggplot(aes(x = geno, y = ed50)) +
  geom_point() +
  geom_errorbar(aes(ymin = ed50 - std.error, ymax = ed50 + std.error)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Genotype", y = "Tolerance / ED50 (°C)",
       title = "RRT A. palmata thermal tolerance")
```
