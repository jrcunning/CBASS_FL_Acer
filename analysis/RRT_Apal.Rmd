...
b1log <- readlog("data/20201006_RRT_Apal/temperature/sdlog/20201007_BOX1_LOG.TXT", prefix = "B1")
b2log <- readlog("data/20201006_RRT_Apal/temperature/sdlog/20201006_BOX2_LOG.TXT", prefix = "B2")
b3log <- readlog("data/20201006_RRT_Apal/temperature/sdlog/20201007_BOX3_LOG.TXT", prefix = "B3")
...
hobologfiles <- list.files("data/20201006_RRT_Apal/temperature/hobo", pattern = ".xlsx", full.names = TRUE)
...
path <- "data/20201006_RRT_Apal/PAM/dpii.xlsx"
dpii <- path %>%
  excel_sheets() %>%
  set_names() %>%
...
write_csv(dpii, "data/20201006_RRT_Apal/PAM/RRT_Apal_pamdata.csv")
```
...
cmods <- dpiif %>%
  ...
  group_by(group) %>%
  nest(data = c(geno, rec, f, fm, fvfm, tank_id, tank_set, max_temp, rfvfm)) %>%
  mutate( mod = map(data, ~ glm(rfvfm ~ log10(max_temp), data = ., family = "quasibinomial")),
         pred = map(mod, ~ data.frame(lsmeans(., specs = "max_temp", at = list(max_temp = seq(30, 40, 0.1)),
                                   type = "response"))),
         ec50 = map_dbl(mod, ~ 10^(MASS::dose.p(., p = 0.5)))) %>%
