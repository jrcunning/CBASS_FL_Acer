...
lo <- 0.1 / 100
hi <- 99.9 / 100
lohi <- qnorm(c(lo, hi), meanadj, sdadj)
lohi
diff(lohi)
lo * n
hi * n
...
```{r}
# Amount of variance within nurseries relative to population variance
nursvar <- res.adj %>%
  group_by(nursery) %>%
  summarise(svar = sum((ed50_adj - mean(ed50_adj))^2) / n(),
            var = var(ed50_adj)) %>%
  ungroup()

totvar <- var(res$estimate_ed50)

nursvar %>%
  mutate(relvar = var / totvar)
```
...
ggsave("output/fig5.png", width = 190, height = 50, units = "mm")
```
...
top25 <- quantile(adjed50dist, prob = 0.75)
...
cowplot::plot_grid(rc1fig, rc2fig, ncol = 2)



# mean ED50 percentile of most tolerant genet in sample?
percentile <- ecdf(adjed50dist)
sample_ed50s3 <- function(ed50s, n) {
  s <- sample(ed50s, n)
  topgenet <- percentile(max(s))
  return(topgenet)
}
...
rc3 <- res.adj %>%
  ...
  nest(data = ed50_adj) %>%
  crossing(n = 1:25) %>%
  mutate(topgenet = map2(data, n, ~ replicate(100, sample_ed50s3(.x$ed50_adj, .y))),
         mean = map_dbl(topgenet, mean))

rc3fig <- ggplot(rc3, aes(x = n, y = mean, color = nursery)) +
  geom_line() +
  geom_point() +
  scale_y_continuous(limits = c(0, 1)) +
  theme_few() +
  theme(axis.text = element_text(size = 7),
        legend.position = c(0.8, 0.4)) +
  labs(x = "Genets sampled", y = "ED50 percentile\nof most tolerant genet")
```
...
ggsave("output/fig6.png", width = 90, height = 99, units = "mm")
