...
hist1 <- res %>%
  ...
  geom_histogram(aes(fill = nursery), binwidth = 0.1, alpha = 0.4, position = position_stack(reverse = TRUE)) +
...
nurs.fig <- ggplot(res, aes(x = nursery, y = estimate_ed50, fill = nursery, group = nursery)) +
  ...
  geom_text(data = nurs.stats, aes(y = 37.5, label = group), size = 3) +
  geom_text(data = nurs.stats, aes(y = 37.5, label = paste("n=", n)), nudge_x = 0.2, size = 3) +
  coord_flip() +
