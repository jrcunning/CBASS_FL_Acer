...
```{r, fig.height = 4}
#########
# Population range -- 1st to 99th percentile across whole population
pop <- qnorm(c(0.001, 0.999), mean(res.adj$ed50_adj), sd(res.adj$ed50_adj))
poprange <- diff(pop)
# ED50 rarefaction curves from each nursery
sample_ed50s <- function(ed50s, n) {
  nursdist <- rnorm(1000, mean = mean(ed50s), sd = sd(ed50s))
  s <- sample(nursdist, n)
  samplerange <- diff(range(s))
  proprange <- samplerange / poprange
  return(proprange)
}

rc <- res.adj %>%
  filter(between(ed50_adj, pop[1], pop[2])) %>%
...
rc1fig <- ggplot(rc, aes(x = n, y = mean, color = nursery)) +
  ...
  theme(text = element_text(size = 8),
        legend.position = "none") +
  labs(x = "Genets sampled", y = "Proportion of population ED50 range captured")
...
#########
# Population range -- 1st to 99th percentile across whole population
pop <- qnorm(c(0.01, 0.99), mean(res.adj$ed50_adj), sd(res.adj$ed50_adj))
poprange <- diff(pop)
f <- res.adj$ed50_adj[between(res.adj$ed50_adj, pop[1], pop[2])]
fpop <- qnorm(c(0.01, 0.99), mean(f), sd(f))
fpoprange <- diff(fpop)
# ED50 rarefaction curves from each nursery
sample_ed50s <- function(ed50s, n) {
  s <- sample(ed50s, n)
  samplerange <- diff(range(s))
  proprange <- samplerange / fpoprange
  return(proprange)
}

rc <- res.adj %>%
  filter(between(ed50_adj, fpop[1], fpop[2])) %>%
  select(nursery, ed50_adj) %>%
  nest(data = ed50_adj) %>%
  crossing(n = 1:25) %>%
