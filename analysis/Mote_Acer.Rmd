...
#write_csv(templog, "data/20201011_Mote_Acer/temperature/Mote_tempdata.csv")
```
...
dpii <- full_join(dpii, tanks) %>%
  select(geno, f, fm, fvfm, max_temp, tank_id, tank_set, cooler_side)
```
...
df <- dpii %>%
  mutate(nursery = "mote") %>%
  select(nursery, geno, f, fm, fvfm, max_temp, tank_id, tank_set, cooler_side)
...
df <- df %>%
  ...
  mutate(fvfm = case_when(
    ...
    problem == "none" ~ fvfm)) %>%
  select(nursery, geno, f, fm, fvfm, max_temp, tank_id, tank_set, cooler_side, fvfmraw, problem)
```
...
df_pos <- bind_rows(Pmap, Smap) %>%
  ...
  mutate(rows_from_top = recode(Row, "A" = 1, "B" = 2, "C" = 3, "D" = 4, "E" = 5, "F" = 6)) %>%
...
ggplot(df_pos, aes(x = rows_from_ctr, y = fvfm, group = cooler, color = tank_set)) +
  ...
  stat_cor(aes(label = ..p.label..)) +
  facet_wrap(~max_temp, labeller = "label_both")
ggplot(df_pos, aes(x = rows_from_top, y = fvfm, group = cooler, color = tank_set)) +
  geom_jitter(alpha = 0.4, width = 0.1) +
  geom_smooth(method = "lm", se = FALSE) +
  stat_cor(aes(label = ..p.label..)) +
  facet_wrap(~max_temp, labeller = "label_both")
...
mod <- lm(fvfm ~ tank_id * cols_from_ctr * rows_from_ctr, 
          data = df_pos, na.action = na.exclude)
...
df <- left_join(df, df_pos) %>%
  select(nursery, geno, f, fm, fvfmraw, fvfm, fvfm.adj, max_temp, problem) %>%
  # Set fvfm values for no or low signal datapoints to zero
  mutate(fvfm = case_when(problem == "no signal" ~ NA_real_,
                          problem == "abnormally high w/ low Ft" ~ NA_real_,
                          problem == "none" ~ fvfm),
         fvfm.adj = case_when(problem == "no signal" ~ NA_real_,
                          problem == "abnormally high w/ low Ft" ~ NA_real_,
                          problem == "none" ~ fvfm.adj))
