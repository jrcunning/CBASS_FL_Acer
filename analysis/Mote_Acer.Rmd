---
title: "Mote *A. cervicornis* CBASS run"
subtitle: "R/V *Coral Reef II*, October 11th, 2020"
...
b1log <- readlog("data/raw/20201011_Mote_Acer/temperature/sdlog/20201011_BOX1_LOG.TXT", prefix = "B1")
b2log <- readlog("data/raw/20201011_Mote_Acer/temperature/sdlog/20201011_BOX2_LOG.TXT", prefix = "B2")

# Tidy arduino log files for this run
arduinolog <- bind_rows(b1log, b2log) %>%
  # Filter to just the date of this run
  filter(date(dttm) == "2020-10-11") %>%
  # Specify which arduino boxes controlled port vs. starboard tank sets
  mutate(tank_set = if_else(str_detect(tank, "B2"), "port", "starboard")) %>%
...
hobologfiles <- list.files("data/raw/20201011_Mote_Acer/temperature/hobo", pattern = ".xlsx", full.names = TRUE)
...
inkbird_sp <- read_csv("data/program_files/inkbird_settings.csv") %>%
  ...
  mutate(max_temp = as.numeric(str_extract(max_temp, "[0-9]+")),
         dttm = ymd_hms(paste("2020-10-11", time)), hm = format(dttm, "%H:%M"),
         temp = NA) %>%
  left_join(read_csv("data/tank_setup.csv")) %>%
  select(dttm, hm, sp, temp, max_temp, tank_set, tank_id)
...
ggplot(templog, mapping = aes(x = dttm, y = temp, group = tank_id)) +
  ...
  scale_x_datetime(breaks = "hours", labels = label_date("%H:%M"),
                   limits = as.POSIXct(c("2020-10-11 12:00:00", "2020-10-11 21:00:00"), tz = "UTC")) +
  theme_hc() +
...
path <- "data/raw/20201011_Mote_Acer/PAM/20201011dii_Acer.xlsx"

dpii <- path %>%
  excel_sheets() %>%
  set_names() %>%
  map_df(~ read_excel(path = path, sheet = .x, range = "A1:H41", col_types = "text"), .id = "tank_id") %>%
...
out <- dpii %>%
  ...
  mutate(mahal = tidy_mahalanobis(f, fvfm),
         is_out = mahal > median(mahal) & f < median(f) & fvfm > median(fvfm, na.rm = TRUE))
...
Pmap <- read_csv("data/raw/20201011_Mote_Acer/P_map.csv") %>%
  pivot_longer(-Row, values_to = "geno", names_to = "Col") %>%
  drop_na() %>%
  unite(pos, Row, Col, sep = "", remove = FALSE)
Smap <- read_csv("data/raw/20201011_Mote_Acer/S_map.csv") %>%
  pivot_longer(-Row, values_to = "geno", names_to = "Col") %>%
  drop_na() %>%
  unite(pos, Row, Col, sep = "", remove = FALSE)
...
# pos.effects <- function(x, xlab) {
#   ggplot(df_pos, aes(x = get(x), y = fvfm, group = cooler, color = tank_set)) +
...
# }
# col.effects <- pos.effects("cols_from_ctr", xlab = "Columns from center")
# row.effects <- pos.effects("rows_from_ctr", xlab = "Rows from center")
# 
# cowplot::plot_grid(col.effects, row.effects)
...
res <- df_pos %>%
  select(tank_id, geno, cols_from_ctr, rows_from_ctr, fvfm) %>%
  # Nest data for each tank individually
  nest(data = c(geno, cols_from_ctr, rows_from_ctr, fvfm)) %>%
  mutate(dataf = map(data, ~ drop_na(.)),
         # Fit a linear model testing crossed effects of rows from center and columns from center
         mod = map(dataf, ~ lm(fvfm ~ rows_from_ctr * cols_from_ctr, data = .x)),
         # Perform model selection to determine if any of these predictors are significant
         mod2 = map2(dataf, mod, ~ step(.y, method = "backwards")),
         # Calculate estimated marginal means across only significant predictors
         emm = map(mod2, ~ emmeans(., specs = as.formula(str_extract(formula(.), "~.*")))),
         emmean = map_dbl(emm, ~ pull(as_data_frame(.), emmean)),
         # Get residuals from model including only significant predictors
         out = map2(mod2, dataf, ~ augment(.x, .y)))

sigterms <- res %>%
  mutate(sigterms = map(mod2, ~ labels(terms(formula(.))))) %>%
  unnest(c(tank_id, sigterms)) %>%
  select(tank_id, sigterms) %>%
  mutate(sigterms = recode(sigterms, "rows_from_ctr" = "row", "cols_from_ctr" = "col",
                           "rows_from_ctr:cols_from_ctr" = "row:col")) %>%
  group_by(tank_id) %>%
  summarize(sigterms = paste0(sigterms, collapse = ",")) %>%
  mutate(sigterms = recode(sigterms, "row,col,row:col" = "row:col")) %>%
  left_join(tanks) %>%
  rename(target_temp = max_temp)
...
ggplot(df_pos, aes(x = cols_from_ctr, y = fvfm, group = rows_from_ctr, 
  ...
  geom_text(data = sigterms, aes(x = 2, y = 0.75, label = sigterms), inherit.aes = FALSE)

# ggplot(df_pos, aes(x = Col, y = fvfm, group = Row, 
#                    color = factor(Row), shape = factor(Row))) +
#   geom_point(alpha = 0.7) +
#   geom_smooth(se = FALSE) +
#   facet_grid(tank_set~target_temp)


# Calculate adjusted fvfm as estimated marginal mean plus residual, only with significant predictors
df_pos <- res %>% 
  select(tank_id, emmean, out) %>%
  unnest(c(emmean, out)) %>%
  mutate(fvfm.adj = emmean + .resid) %>%
  select(geno, fvfm, fvfm.adj, tank_id, cols_from_ctr, rows_from_ctr)
...
calc.ss <- finalmods %>%
  filter(geno != "ML5") %>%    # Correct for this after finding ML5 in genotype map of cooler
...
final %>%
  ...
  drop_na() %>%
  mutate(date = "2020-10") %>%
  write_csv("data/processed/mote_fvfm_adj_clean.csv")
```