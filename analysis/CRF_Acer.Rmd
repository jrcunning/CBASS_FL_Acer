---
title: "CRF *A. cervicornis* CBASS run"
subtitle: "R/V *Coral Reef II*, October 13th, 2020"
...
#write_csv(templog, "data/20201013_CRF_Acer/temperature/CRF_tempdata.csv")
```
...
dpii <- full_join(dpii, tanks) %>%
  select(geno, f, fm, fvfm, tank_id, tank_set, max_temp, cooler_side)
```
...
test <- bind_rows(Pmap, Smap) %>%
  full_join(dpii) %>%
  mutate(Row = factor(Row, levels = c("F", "E", "D", "C", "B", "A")))


test %>%
  group_by(tank_id, Col, Row) %>%
  summarize(fvfm = mean(fvfm, na.rm = TRUE)) %>%
  left_join(distinct(test, tank_id, Col, Row, tank_set, max_temp, cooler_side), by = c("tank_id", "Col", "Row")) %>%
  ungroup() %>%
  ...
  filter(cooler == "s_cooler_1") %>%
  ggplot(aes(x = Col, y = Row, color = fvfm)) + 
  geom_point(size = 5) +
  scale_color_gradient(low = "red", high = "blue") +
  facet_grid(cooler~cooler_side)


test <- test %>%
  mutate(cols_from_ctr = case_when(
    cooler_side == "right" ~ recode(Col, "5" = "4"),
    cooler_side == "left" ~ recode(Col, "5" = "1", "4" = "1", "3" = "2", "2" = "3", "1" = "4"))) %>%
  mutate(cols_from_ctr = as.numeric(cols_from_ctr)) %>%
  mutate(rows_from_ctr = case_when(Row == "A" ~ 3, Row == "B" ~ 2, Row == "C" ~ 1, Row == "D" ~ 1,
                                   Row == "E" ~ 2, Row == "F" ~ 3))
  

ggplot(test, aes(x = cols_from_ctr, y = fvfm, group = max_temp)) +
...
ggplot(test, aes(x = rows_from_ctr, y = fvfm, group = max_temp)) +
...
test <- test %>%
  mutate(max_temp = factor(max_temp))

mod <- lm(fvfm ~ max_temp * cols_from_ctr * rows_from_ctr, data = test)
summary(mod)
anova(mod)
visreg(mod, xvar = "rows_from_ctr", by = "max_temp")

res <- emmeans(mod, specs  = c("cols_from_ctr", "rows_from_ctr"), by = "max_temp") %>% 
  rbind() %>%
  as_tibble() %>%
  select(max_temp,)
...
fvals %>%
  ...
  drop_na() %>%
  mutate(date = "2020-10") %>%
  write_csv("data/processed/crf_fvfm_clean.csv")
```
