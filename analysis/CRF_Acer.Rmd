...
test <- df %>%
  filter(max_temp >= 36) %>%
  #drop_na(fvfm) %>%
  replace_na(replace = list(fvfm = 0, f = 0, fm = 0)) %>%
  mutate(mahal = tidy_mahalanobis(f, fvfm))
median(test$mahal)
ggplot(test, aes(x = f, y = fvfm)) +
  geom_point(aes(color = mahal > 1.8, shape = factor(max_temp))) +
...
test2 <- df %>%
  ...
  group_by(max_temp) %>%
  mutate(mahal = tidy_mahalanobis(f, fvfm),
         is_out = mahal > median(mahal) & fvfm > median(fvfm) & f < median(f))
ggplot(test2, aes(x = f, y = fvfm)) +
...
df <- df %>%
  ...
  replace_na(replace = list(fvfm = 0, f = 0)) %>%   
  # Merge with mahalanobis outlier info
  left_join(out) %>%
