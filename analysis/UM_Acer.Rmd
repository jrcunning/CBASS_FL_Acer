...
b1log <- readlog("data/raw/20201014_UM_Acer/temperature/sdlog/20201014_BOX1_LOG.TXT", prefix = "B1")
b3log <- readlog("data/raw/20201014_UM_Acer/temperature/sdlog/20201014_BOX3_LOG.TXT", prefix = "B3")
...
hobologfiles <- list.files("data/raw/20201014_UM_Acer/temperature/hobo", pattern = ".xlsx", full.names = TRUE)
...
path <- "data/raw/20201014_UM_Acer/PAM/20201014dii_Acer.xlsx"
dpii <- path %>%
  excel_sheets() %>%
  set_names() %>%
...
df <- df %>%
  ...
  mutate(fvfm = case_when(
    ...
    problem == "none" ~ fvfm)) %>%
  select(nursery, geno, f, fm, fvfm, max_temp, target_temp, tank_id, tank_set, cooler_side, fvfmraw, problem)
```

# Adjust for positional effects
```{r}
# Import maps of the position of each genotype in each tank
Pmap <- read_csv("data/raw/20201014_UM_Acer/P_map.csv") %>%
  pivot_longer(-Row, values_to = "geno", names_to = "Col") %>%
  drop_na() %>%
  unite(pos, Row, Col, sep = "", remove = FALSE)
Smap <- read_csv("data/raw/20201014_UM_Acer/S_map.csv") %>%
  pivot_longer(-Row, values_to = "geno", names_to = "Col") %>%
  drop_na() %>%
  unite(pos, Row, Col, sep = "", remove = FALSE)
...
df_pos %>% filter(tank_set == "starboard", target_temp == 38, cols_from_ctr == 1)
...
initmods <- df %>%
  nest(data = c(target_temp, max_temp, f, fm, fvfmraw, fvfm, problem)) %>%
...
fmods <- dff %>%
  select(nursery, geno, target_temp, max_temp, f, fm, fvfmraw, problem, fvfm) %>%
  nest(data = c(target_temp, max_temp, f, fm, fvfmraw, fvfm, problem)) %>%
  # Fit the model to each coral
  mutate(ll3 = map(data, tryll3)) %>%
  # Get model parameters and fitted values/residuals
...
fvals <- fmods %>%
  ...
  unnest(pred) %>%
  full_join(fed50) %>%
  full_join(select(dff, nursery, geno, target_temp, max_temp, f, fm, fvfmraw, problem, fvfm)) %>%
  rename(ed50 = estimate)
```
...
fvals %>%
  ...
  drop_na() %>%
  mutate(date = "2020-10") %>%
  write_csv("data/processed/um_fvfm_adj_clean.csv")
```
