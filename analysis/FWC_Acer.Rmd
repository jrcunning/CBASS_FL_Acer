---
title: "FWC *A. cervicornis* CBASS run"
subtitle: "R/V *Coral Reef II*, October 10th, 2020"
...
b1log <- readlog("data/raw/20201010_FWC_Acer/temperature/sdlog/20201010_BOX1_LOG.TXT", prefix = "B1")
b2log <- readlog("data/raw/20201010_FWC_Acer/temperature/sdlog/20201010_BOX2_LOG.TXT", prefix = "B2")

# Tidy arduino log files for this run
arduinolog <- bind_rows(b1log, b2log) %>%
  # Filter to just the date of this run
  filter(date(dttm) == "2020-10-10") %>%
...
hobologfiles <- list.files("data/raw/20201010_FWC_Acer/temperature/hobo", pattern = ".xlsx", full.names = TRUE)
...
inkbird_sp <- read_csv("data/program_files/inkbird_settings.csv") %>%
  ...
  mutate(max_temp = as.numeric(str_extract(max_temp, "[0-9]+")),
         dttm = ymd_hms(paste("2020-10-10", time)), hm = format(dttm, "%H:%M"),
         temp = NA) %>%
  left_join(read_csv("data/tank_setup.csv")) %>%
  select(dttm, hm, sp, temp, max_temp, tank_set, tank_id)
...
ggplot(templog, mapping = aes(x = dttm, y = temp, group = tank_id)) +
  ...
  scale_x_datetime(breaks = "hours", labels = label_date("%H:%M"),
                   limits = as.POSIXct(c("2020-10-10 12:00:00", "2020-10-10 21:00:00"), tz = "UTC")) +
  theme_hc() +
  theme(legend.position = "none", axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Time of day", y = "Temperature (°C)")

#write_csv(templog, "data/20201010_FWC_Acer/temperature/FWC_tempdata.csv")
```
...
path <- "data/raw/20201010_FWC_Acer/PAM/20201010_dii_bytank.xlsx"
dpii <- path %>%
  excel_sheets() %>%
  set_names() %>%
  map_df(~ read_excel(path = path, sheet = .x, range = "A1:H27", col_types = "text"), .id = "tank_id") %>%
...
# Filter out one genotype of Apal included in this run
dpii <- dpii %>% 
  filter(!geno == "ApalCCN4")
```
...
Pmap <- read_csv("data/raw/20201010_FWC_Acer/P_map.csv") %>%
  pivot_longer(-Row, values_to = "geno", names_to = "Col") %>%
  drop_na() %>%
  unite(pos, Row, Col, sep = "", remove = FALSE)
Smap <- read_csv("data/raw/20201010_FWC_Acer/S_map.csv") %>%
  pivot_longer(-Row, values_to = "geno", names_to = "Col") %>%
  drop_na() %>%
  unite(pos, Row, Col, sep = "", remove = FALSE)
...
fvals %>%
  ...
  drop_na() %>%
  mutate(date = "2020-10") %>%
  write_csv("data/processed/FWC_fvfm_adj_clean.csv")
```
