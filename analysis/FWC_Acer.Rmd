...
df_pos <- bind_rows(Pmap, Smap) %>%
  ...
  mutate(cols_from_ctr = case_when(
    cooler_side == "right" ~ recode(Col, "5" = 4, "4" = 3, "3" = 2, "2" = 1, "1" = 1),
    cooler_side == "left" ~ recode(Col, "5" = 1, "4" = 1, "3" = 2, "2" = 3, "1" = 4))) %>%
  # Recode cooler row position as number of rows from center of cooler
  mutate(rows_from_ctr = case_when(Row == "A" ~ 2, Row == "B" ~ 2, Row == "C" ~ 1, 
                                   Row == "D" ~ 1, Row == "E" ~ 2, Row == "F" ~ 2))
...
dff <- vals %>%
  ...
  mutate(max_to_remove = floor(n * 0.1)) %>%
  ungroup() %>%
